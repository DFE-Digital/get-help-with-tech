...
class Computacenter::AssociateSettingsToOrdersService < ApplicationService
  def call
    ids_ok = Computacenter::Order.joins(:school, :responsible_body).pluck(:id).uniq
    scope_ids = Computacenter::Order.pluck(:id) - ids_ok
...
private
  ...
  def school_from_ship_to(ship_to)
    School.find_by_computacenter_reference(ship_to)
  end

  def school_from_ship_to_customer(ship_to_customer)
    School.where("concat(urn, ' ', name) = ?", ship_to_customer).first
  end

  def school_from_responsible_body(responsible_body)
    School.find_by_name(responsible_body)
  end
  ...
  def rb_sold_to(order, school)
    return order.sold_to if ResponsibleBody.exists?(computacenter_reference: order.sold_to)

    school&.responsible_body_computacenter_reference
  end
end
