...
private
  ...
  def message_type_for_school(school)
    if %w[rb_can_order].include?(school.device_ordering_status) && school.responsible_body.has_virtual_cap_feature_flags?
      :user_can_order_in_virtual_cap
    elsif %w[rb_can_order school_can_order].include?(school.device_ordering_status)
      if school.responsible_body.new_fe_wave?
        :user_can_order_in_fe_college
      else
        :user_can_order
      end
    elsif %w[needs_info].include?(school.device_ordering_status)
