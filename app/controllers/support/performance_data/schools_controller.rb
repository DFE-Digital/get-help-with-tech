  ...
  def school_query
    <<-SQL
      SELECT  schools.name AS school_name,
              ...
              CASE schools.order_state WHEN 'cannot_order' THEN schools.raw_laptops_ordered ELSE schools.raw_laptop_cap END AS cap,
              schools.raw_laptops_ordered AS devices_ordered,
              schools.raw_router_allocation AS coms_allocation,
              CASE schools.order_state WHEN 'cannot_order' THEN schools.raw_routers_ordered ELSE schools.raw_router_cap END AS coms_cap,
      ...
      WHERE (CASE schools.order_state WHEN 'cannot_order' THEN schools.raw_laptops_ordered ELSE schools.raw_laptop_cap END) > 0
          OR schools.raw_laptop_allocation > 0
          OR (CASE schools.order_state WHEN 'cannot_order' THEN schools.raw_routers_ordered ELSE schools.raw_router_cap END) > 0
