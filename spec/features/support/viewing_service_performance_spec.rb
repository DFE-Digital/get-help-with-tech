...
RSpec.feature 'Viewing service performance', type: :feature do
  ...
  def given_there_are_available_shipped_and_remaining_devices
    devolved_schools = create_list(:school, 5, :manages_orders, laptops: [1, 1, 0])
    managed_schools = create_list(:school, 5, :centrally_managed, laptops: [1, 1, 0])

    UpdateSchoolDevicesService.new(school: devolved_schools[0], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 30).call
    UpdateSchoolDevicesService.new(school: devolved_schools[1], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[2], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[3], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[4], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 0).call

    UpdateSchoolDevicesService.new(school: managed_schools[0], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 30).call
    UpdateSchoolDevicesService.new(school: managed_schools[1], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 30).call
    UpdateSchoolDevicesService.new(school: managed_schools[2], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 10).call
    UpdateSchoolDevicesService.new(school: managed_schools[3], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 0).call
    UpdateSchoolDevicesService.new(school: managed_schools[4], order_state: :can_order, laptop_allocation: 30, laptops_ordered: 0).call
  end

  def given_there_are_available_shipped_and_remaining_routers
    devolved_schools = create_list(:school, 5, :manages_orders, routers: [1, 1, 0])
    managed_schools = create_list(:school, 5, :centrally_managed, routers: [1, 1, 0])

    UpdateSchoolDevicesService.new(school: devolved_schools[0], order_state: :can_order, router_allocation: 30, routers_ordered: 30).call
    UpdateSchoolDevicesService.new(school: devolved_schools[1], order_state: :can_order, router_allocation: 30, routers_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[2], order_state: :can_order, router_allocation: 30, routers_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[3], order_state: :can_order, router_allocation: 30, routers_ordered: 10).call
    UpdateSchoolDevicesService.new(school: devolved_schools[4], order_state: :can_order, router_allocation: 30, routers_ordered: 0).call

    UpdateSchoolDevicesService.new(school: managed_schools[0], order_state: :can_order, router_allocation: 30, routers_ordered: 30).call
    UpdateSchoolDevicesService.new(school: managed_schools[1], order_state: :can_order, router_allocation: 30, routers_ordered: 30).call
    UpdateSchoolDevicesService.new(school: managed_schools[2], order_state: :can_order, router_allocation: 30, routers_ordered: 10).call
    UpdateSchoolDevicesService.new(school: managed_schools[3], order_state: :can_order, router_allocation: 30, routers_ordered: 0).call
    UpdateSchoolDevicesService.new(school: managed_schools[4], order_state: :can_order, router_allocation: 30, routers_ordered: 0).call
  end
