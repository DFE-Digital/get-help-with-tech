...
RSpec.feature 'Ordering devices' do
  ...
  let(:schools) { create_list(:school, 6, :manages_orders, :with_headteacher, laptops: [1, 1, 0], responsible_body:) }
  let(:user) { create(:local_authority_user, :with_a_confirmed_techsource_account, responsible_body:, orders_devices: true) }
  let(:user_no_ts) { create(:local_authority_user, responsible_body:, orders_devices: false) }
  let(:user_awaiting_ts) { create(:local_authority_user, responsible_body:, orders_devices: true) }
