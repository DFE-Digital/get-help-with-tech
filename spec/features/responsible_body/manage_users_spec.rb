...
RSpec.feature 'Managing ResponsibleBody users' do
  ...
  let!(:rb_user_2) { create(:local_authority_user, responsible_body: rb_user.responsible_body) }
  let!(:user_from_other_rb) { create(:trust_user) }
  let(:rb_users_index_page) { PageObjects::ResponsibleBody::UsersPage.new }
  let(:new_rb_user_form) { PageObjects::ResponsibleBody::NewUserForm.new }
  ...
  context 'clicking "Invite a new user"' do
    ...
    it 'shows the new user form' do
      ...
      expect(page).to have_field 'Can they order devices for schools?'
    end
    ...
    it 'adds the user when I submit the form with all required fields' do
      ...
      within("Can they order devices for schools?") do
        choose('Yes, create a TechSource account for them')
      end
      click('Send invite')
