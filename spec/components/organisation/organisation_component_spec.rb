...
RSpec.describe Organisation::OrganisationComponent, type: :component do
  ...
  subject { render_inline(described_class.new(organisation: organisation)) }
  ...
  context 'independent special school' do
    let(:organisation) { build(:iss_provision, responsible_body: responsible_body) }
  ...
  context 'social care leaver' do
    let(:organisation) { build(:scl_provision, responsible_body: responsible_body) }
