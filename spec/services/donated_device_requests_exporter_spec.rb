...
RSpec.describe DonatedDeviceRequestsExporter, type: :model do
  ...
  let(:laptops_request) { create(:donated_device_request, :complete, :wants_laptops, schools: [schools[0].id], responsible_body: schools[0].responsible_body) }
  let(:tablets_request) { create(:donated_device_request, :complete, :wants_tablets, schools: [schools[1].id], responsible_body: schools[1].responsible_body) }
  ...
  context 'when given a filename' do
    ...
    it 'includes the request information in the CSV file' do
      CSV.read(filename, headers: true).each do |request|
        ...
        expect(record.created_at.to_s).to eq(request['created_at'])
        expect(school.urn.to_s).to eq(request['urn'])
        expect(school.computacenter_reference).to eq(request['shipTo'])
        expect(record.responsible_body.computacenter_reference).to eq(request['soldTo'])
