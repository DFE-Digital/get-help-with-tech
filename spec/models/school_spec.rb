...
RSpec.describe School, type: :model do
  ...
  describe '#perceived_state' do
    subject(:school) { create(:school, responsible_body: rb, std_device_allocation: allocation) }
    ...
    context 'when RB orders' do
      context 'when devices can be ordered' do
        ...
        before do
          school.update(preorder_information: create(:preorder_information, :rb_will_order, status: 'ready'))
        end

        it 'returns rb_can_order' do
          expect(school.perceived_state).to eql('rb_can_order')
        end
      end

      context 'when no devices to order' do
        before do
          school.update(preorder_information: create(:preorder_information, :rb_will_order, status: 'ready'))
        end

        it 'returns ready' do
          expect(school.perceived_state).to eql('ready')
        end
      end
    end

    context 'when school orders' do
      before do
        school.update(preorder_information: create(:preorder_information, :school_will_order, status: 'ready'))
      end

      context 'when devices can be ordered' do
        let(:allocation) { create(:school_device_allocation, :with_orderable_devices) }

        it 'returns school_can_order' do
          expect(school.perceived_state).to eql('school_can_order')
        end
      end
    ...
    context 'when school not in a ready state' do
      before do
        school.update(preorder_information: create(:preorder_information, :rb_will_order, status: 'needs_contact'))
      end

      it 'returns preorder status' do
        expect(school.perceived_state).to eql('needs_contact')
      end
    end
  end
end
